...
var FiveBeansWorker = function(options)
{
    ...
    this.maxReserves   = options.maxReserves || 30;
...
FiveBeansWorker.prototype.runJob = function(jobID, job)
{
    ...
    self.client.stats_job(jobID, function (err, res) {
        if (res.reserves > self.maxReserves) {
            self.emitWarning({ message: 'reserves > ' + self.maxReserves.toString() + ' times and buried', id: jobID, type: job.type });
            self.buryAndMoveOn(jobID);
        }
        else {
